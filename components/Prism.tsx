...
export default function Prism({
  ...
  useEffect(() => {
    ...
    const resize = () => {
      ...
      (program.uniforms.uPxScale as { value: number }).value =
    ...
    if (animationType === "hover") {
      ...
      (program.uniforms.uUseBaseWobble as { value: number }).value = 0;
    } else if (animationType === "3drotate") {
      (program.uniforms.uUseBaseWobble as { value: number }).value = 0;
    } else {
      (program.uniforms.uUseBaseWobble as { value: number }).value = 1;
    }
    ...
    const render = (t: number) => {
      ...
      (program.uniforms.iTime as { value: number }).value = time;
      ...
      if (animationType === "hover") {
        ...
        (program.uniforms.uRot as { value: Float32Array }).value = setMat3FromEuler(
          yaw,
          pitch,
          roll,
          rotBuf
        ...
        rotBuf[7] = 0;
        rotBuf[8] = 1;
        (program.uniforms.uRot as { value: Float32Array }).value = rotBuf;
        if (TS < 1e-6) continueRAF = false;
      }
